...
from ..modules.host_http import HostHttpClient
    ...
    def _find_download_history(self, download_hash: str) -> dict[str, Any] | None:
        for page in range(1, self.settings.host_history_download_max_pages + 1):
            payload = self.client.get_json(
                ...
                params={"page": page, "count": self.settings.host_history_download_page_size},
                auth_mode="x_api_key",
            )
    ...
    def _find_transfer_history(self, download_hash: str) -> dict[str, Any] | None:
        for page in range(1, self.settings.host_history_transfer_max_pages + 1):
            payload = self.client.get_json(
                ...
                params={"page": page, "count": self.settings.host_history_transfer_page_size},
                auth_mode="x_api_key",
            )
            data = payload.get("data") if isinstance(payload.get("data"), dict) else {}
            items = data.get("list") if isinstance(data.get("list"), list) else []
            for item in items:
                if isinstance(item, dict) and str(item.get("download_hash") or "") == download_hash and item.get("status", True) is not False:
                    return item
