"""Persistent settings helper aligned with MoviePilot-style support layers."""
...
class SettingsHelper:
    def __init__(self, session: Session, env_settings: Any = settings):
        ...
        self.oper = SettingsOper(session)

    def provider_settings(self) -> ProviderSettingsResponse:
        chart_provider_mode = self.oper.get_value("chart_provider_mode")
        chart_provider_mode = self._resolve_chart_provider_mode(chart_provider_mode)

        chart_rss_feeds = self.oper.get_value("chart_rss_feeds")
    ...
    def update_provider_settings(
        ...
        self.oper.set_value("chart_provider_mode", payload.chart_provider_mode.value)
        self.oper.set_value(
            "chart_rss_feeds",
    ...
    def rule_profiles(self) -> list[RuleProfile]:
        stored_profiles = self.oper.get_value("rule_profiles")
    ...
    def _resolve_chart_provider_mode(self, stored_value: Any) -> ChartProviderMode:
        ...
        try:
            ...
            logger.warning(
                "Invalid stored chart_provider_mode %r, falling back to env value %r.",
                stored_value,
                fallback_value,
            )
            try:
