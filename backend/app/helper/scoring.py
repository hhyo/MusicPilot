...
class MusicCandidateScorer:
    def score(
        ...
        *,
        ...
        breakdown = {
            "title_match": ScoreBreakdownItem(score=title_match, reason="标题命中度，基于 metadata 主标题与候选标题的分词比对。"),
            "artist_match": ScoreBreakdownItem(score=artist_match, reason="艺人命中度，优先匹配主艺人名称。"),
            "release_match": ScoreBreakdownItem(score=release_score, reason="专辑 / 歌曲 / 艺人上下文匹配度。"),
            "year_match": ScoreBreakdownItem(score=year_score, reason="年份命中度，当前只做标题内年份比对。"),
            "format_quality": ScoreBreakdownItem(score=format_score, reason="格式偏好得分，lossless / hi-res 高于有损格式。"),
            "bitrate_preference": ScoreBreakdownItem(score=bitrate_score, reason="码率与无损偏好得分。"),
            "seeder_availability": ScoreBreakdownItem(score=seeder_score, reason="做种数得分，优先更容易下载的候选。"),
            "negative_keyword_penalty": ScoreBreakdownItem(
                score=-negative_penalty,
                reason=(f"负向关键词惩罚，命中 {', '.join(negative_hits)}。" if negative_hits else "未命中负向关键词。"),
            ),
        }
        ...
        decision = self._resolve_decision(score_total=score_total, preferences=preferences)
