...
- Chart discovery：当前支持 `mock` 与 `listenbrainz` 两种模式。`listenbrainz` 第一版已接入 sitewide artists / recordings 榜单；discovery 页面现已提供榜单摘要、featured entry、分组视图，以及稳定的 `discovery -> metadata` 转化桥接层。自动刷新、增量监控和专辑榜仍待后续补齐。
