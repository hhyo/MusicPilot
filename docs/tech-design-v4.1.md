...
> **版本**: v4.1（融合之前的订阅设计）
> **变更**: 融合 M5 订阅功能（艺术家/专辑）+ M4 PT站点搜索/下载器对接
> **最后更新**: 2026-02-26
...
- **流程**：
  ```
  查询 MusicBrainz -> 发现新专辑 -> PT 站点搜索 -> 推送下载
  ```
...
- **流程**：
  ```
  查询 MusicBrainz -> 发现新版本 -> PT 站点搜索 -> 推送下载
  ```
...
class Site(Base, TimestampMixin):
    ...
    # 站点类型
    site_type: Mapped[str] = mapped_column(String(50), default="pt")  # pt, other
```
...
| 任务 | 描述 | 预计工时 |
|------|------|----------|
| M4-T1 | PT站点数据模型（Site） | 3h |
| M4-T2 | TorrentsChain 资源链 | 8h |
| M4-T3 | 站点模块（MTeam/HDChina） | 10h |
...
所有三种订阅类型都遵循统一的工作流程：
```
检查更新 -> PT站点搜索 -> 匹配规则 -> 推送下载 -> 监控进度 -> 整理文件
```