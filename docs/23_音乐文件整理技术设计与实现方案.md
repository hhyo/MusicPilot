...
- 独立的 `MusicMetadataRecognizer / MusicLayoutPlanner` 还没有单独拆成新模块
...
当前仓库已经把原先混在 `OrganizeStrategyService` 里的“音乐元数据识别”逻辑拆成了独立的 `MusicMetadataRecognizer`：

- `MusicMetadataRecognizer`
...
这一步的目标不是增强元数据来源，而是先把“音乐元数据识别”和“路径规划”拆成两个清晰边界，方便后续再独立演进 `MusicLayoutPlanner`。

当前第一轮增强已经落地到 `MusicMetadataRecognizer`：

- 保留 `MetadataDetail` 作为最高优先级
- 保留 `candidate` 现有基础字段兜底
- 新增利用 `source_path` 的文件名、父目录、祖父目录做最小音乐元数据识别

当前第二轮增强也已经落地到 `MusicMetadataRecognizer`：

- 在 `source_path` 指向真实本地音频文件时，尝试读取嵌入音频标签
- 标签优先级位于显式 `MetadataDetail` / `raw_payload` 之后、路径 hints 之前
- 当前最小标签集只覆盖 `title / artist / album / date(year)`

当前这轮增强只做：

- `grandparent_dir -> artist`
- `YYYY - Album -> year + album`
- `NN - Title -> track title`

当前仍未接入在线 metadata provider，也还没有继续扩展更复杂的多碟/合集/演奏者标签语义。

### 三点七、当前已完成的 layout planner 抽离

当前仓库已经继续把原先留在 `OrganizeStrategyService` 里的“相对路径选择 + 模板渲染”抽成了独立的 `MusicLayoutPlanner`：

- `MusicLayoutPlanner`
  - 负责根据 `entity_type` 选择 artist/album/track 层级
  - 负责模板渲染和 `target_relative_path` 计算
- `OrganizeStrategyService`
  - 继续保留为更薄的 plan shell
  - 只负责 snapshot 构造、metadata resolver 调用、layout planner 调用和最终 `OrganizePlan` 组装

到这一步，音乐 organize 的上层主链已经形成了更清晰的两段：

1. `MusicMetadataRecognizer`
2. `MusicLayoutPlanner`

后续如果继续演进，优先方向就不再是拆边界，而是增强这两层各自的输入能力。
